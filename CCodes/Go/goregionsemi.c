...
unsigned int Owner(unsigned int e, unsigned int statusarea) {
   ...
   if (e == CHIP1 && statusarea == CHIP1) {
      return e;
   }
   if (e == CHIP2 && statusarea == CHIP2) {
      return e;
   }
}
...
unsigned int Analyze(struct GOCell **goboard, unsigned int size, unsigned int x, unsigned int y, struct Queue *q, unsigned int statusarea) {
   unsigned int pos;
   if (x >= 0 && x < size && y >= 0 && y < size && goboard[x][y].statuscell == UNVISITED) {
      if (goboard[x][y].element == NOCHIP) {
         pos = x * size + y;
         EnQueue(q, pos);
      } else {
         unsigned int new_statusarea = Owner(goboard[x][y].element, statusarea);
         if (new_statusarea == NOOWNER) {
            return NOOWNER;
         }
         statusarea = new_statusarea;
      }
   }
   return statusarea;
}
